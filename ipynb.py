...
# todo проверка, что если неисправных в перемешенном меньше чем 10%, то перемешать еще раз
...
df_train[['failure', 'smart_5_raw','smart_187_raw','smart_188_raw','smart_197_raw','smart_198_raw']].describe()

"""Отобразим матрицу диаграмм рассеивания, чтобы установить зависимости между 5 параметрами.  Каждая диагональ матрицы показывает распределение одного параметра, а вне диагонали - диаграммы рассеяния для каждой пары параметров.

Let us display a matrix of scatter diagrams to determine the dependencies among the 5 parameters.  Each diagonal of the matrix shows the distribution of one parameter, and the off-diagonal shows the scatter plots for each pair of parameters.
"""

sns.pairplot(df_train[['smart_5_raw', 'smart_187_raw', 'smart_188_raw', 'smart_197_raw', 'smart_198_raw']])
plt.show()

df_test.head()

df_test[['failure', 'smart_5_raw','smart_187_raw','smart_188_raw','smart_197_raw','smart_198_raw']].describe()

sns.pairplot(df_test[['smart_5_raw', 'smart_187_raw', 'smart_188_raw', 'smart_197_raw', 'smart_198_raw']])
plt.show()

print(f'[ Training sample size ] >>> {df_train.shape}')
print(f'[ Test sample size ] >>> {df_test.shape}')

"""### 1.1 Распределение целевой переменной (Сбои)
### 1.1 Distribution of target variable (Failure)
...
"""### 1.2 Балансировка набора данных
...
print("[ Number of working HDDs ] >>> ", len(valid))        # storing the total number of valid HDDs
print("[ Number of faulty HDDs ] >>> ", len(failed))        # storing the total number of HDDs likely to fail
...
if len(valid) > 0:
    ...
    print("Insufficient data in variable 'valid'")

""" Теперь, объединяем недавно преобразованные классы с обучающими данными.
...
"""Заметим, что размеры набора обучающих данных увеличились вдвое.
...
"""### 1.3 Функция Выбор и заполнение недостающих значений
### 1.3 Feature Selection and filling of missing values
...
"""### 1.4 Разделение значений для X_test и Y_test (ОПЦИОНАЛЬНО)

Заметка: Пропустите этот шаг, если вы используете метод TrainTestSpilt.

### 1.4 Splitting values for X_test and Y_test (OPTIONAL)

Note: Skip this step if using TrainTestSpilt Method
...
"""## 2. Построение модели с использованием Cлучайного Леса

## 2. Building the model using Random Forest

 _________________________________________________________________________

### Модель 1: RF с использованием X_test и Y_test
Примечание. Если вы используете Train_Test_Split, обратитесь к модели 2.
...
print("[ The precision ] >>> {}".format(prec))

rec = recall_score(Y_test, yPred)
print("[ The recall ] >>> {}".format(rec))

f1 = f1_score(Y_test, yPred)
print("[ The F1-Score ] >>> {}".format(f1))

MCC = matthews_corrcoef(Y_test, yPred)
print("[ The Matthews correlation coefficient ] >>> {}".format(MCC))
...
"""### Модель [ 2 ] : RF с использованием метода Train_Test_Split
...
# Get the first tree from the forest
single_tree = rfc.estimators_[50]
